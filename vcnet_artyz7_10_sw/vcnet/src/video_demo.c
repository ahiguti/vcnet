...
void DemoInitialize()
{
	 ...
	 */
	xil_printf("videosetcallback\r\n");
	VideoSetCallback(&videoCapt, DemoISR, &fRefresh);

//	DemoPrintTest(dispCtrl.framePtr[dispCtrl.curFrame], dispCtrl.vMode.width, dispCtrl.vMode.height, dispCtrl.stride, DEMO_PATTERN_1);
//	DemoPrintTest(dispCtrl.framePtr[dispCtrl.curFrame + 1], dispCtrl.vMode.width, dispCtrl.vMode.height, dispCtrl.stride, DEMO_PATTERN_1);
...
{
	return videoCapt.curFrame;
}

void vcnet_video_set_video_frame(u32 vidx, u32 didx)
{
	VideoChangeFrame(&videoCapt, vidx);
	DisplayChangeFrame(&dispCtrl, didx);
}

void vcnet_video_exec_step()
{
	VideoCheckSignal(&videoCapt);
}

int vcnet_video_status()
{
	return videoCapt.state;
}

int vcnet_video_locked()
{
	return videoCapt.locked;
}
