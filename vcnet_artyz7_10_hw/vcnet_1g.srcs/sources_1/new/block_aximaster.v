...
BRESP, BVALID, BREADY, BUSER, BLK_TREADY, BLK_TVALID, BLK_ADDRESS,
BLK_COUNT_M1, BLK_MODE_WRITE, BLK_ERROR, WR_TREADY, WR_TVALID,
WR_TDATA, RD_TREADY, RD_TVALID, RD_TDATA, DEBUG_OUT
);
parameter DATA_BITS = 32;
input wire CLK;
input wire RESET_N;
output wire BLK_TREADY;
input wire BLK_TVALID;
input wire [31:0] BLK_ADDRESS;
input wire [31:0] BLK_COUNT_M1;
input wire BLK_MODE_WRITE;
output wire [1:0] BLK_ERROR;
output wire WR_TREADY;
input wire WR_TVALID;
input wire [DATA_BITS-1:0] WR_TDATA;
input wire RD_TREADY;
output wire RD_TVALID;
output wire [DATA_BITS-1:0] RD_TDATA;
...
assign BLK_TREADY = state_fe;
assign BLK_ERROR = error;
assign WR_TREADY = state_w && WREADY;
assign RD_TVALID = state_r && RVALID;
assign RD_TDATA = RDATA;
...
assign ARPROT = 3'b0;
assign ARQOS = 4'b0;
assign RREADY = state_r && RD_TREADY;
...
assign AWUSER = 0;
assign WID = 6'b0;
assign WDATA = WR_TDATA;
assign WSTRB = 8'hff;
assign WLAST = burst_len == 0;
assign WVALID = state_w && WR_TVALID;
assign WUSER = 0;
...
always @(posedge CLK) begin
    if (!RESET_N) begin
        ...
        if (BLK_TREADY && BLK_TVALID) begin
            state_fe <= 0;
            error <= 0;
            address <= BLK_ADDRESS;
            rep_count <= BLK_COUNT_M1[31:8];
            burst_len <= (BLK_COUNT_M1[31:8] != 0) ? 8'hff : BLK_COUNT_M1[7:0];
            last_burst <= BLK_COUNT_M1[7:0];
            if (BLK_MODE_WRITE) begin
                state_aw <= 1;
                state_w <= 1;
            end else begin
                state_ar <= 1;
